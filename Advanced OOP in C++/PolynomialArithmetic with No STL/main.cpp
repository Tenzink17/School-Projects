...
template<class T>
void print_list(node<T>* p, ofstream &o)
{
    while(p)
    {
        o<<p->coefficent()<<"x^"<<p->exponent()<<" ";
...
int main()
{
    ...
    outfile<<"Read from text file \n";

    outfile<<str[0]<<"\n";

    outfile<<"\n";

    outfile<<"In linked list \n";

    print_list(head, outfile);

    outfile<<"\n";

    bubble_sort(head);

    outfile<<"Sorted by highest power \n";
    ...
    outfile<<"Canonical Form \n";
    print_list(temp,outfile);
